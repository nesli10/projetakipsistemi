            ...
            <div class="row">
              <div class="col-lg-6">
                <div class="d-flex flex-column h-100">
                  ...
                  <form role="form text-left" method="POST" action="{{url('donem-kaydet') }}" enctype="multipart/form-data">
                    @csrf
                    <select class="form-control" name="donem" id="donem">
                      @foreach ($donemler as $item)
                      <option name='{{$item->donem_id}}' value="{{$item->donem}}">{{$item->donem}}</option>
                      @endforeach
                    </select>
                    <button type="submit" class="btn bg-gradient-dark btn-md mt-4 mb-4" name="donemKaydet">KAYDET</button> 
                </form>
                </div>
              </div>
