  ...
  <div class="container-fluid py-4">
    <div class="row">
      ...
      <div class="col-xl-3 col-sm-6">
        <a href="/"><button type="button" class="btn btn-secondary" style="margin-top: 15px">Çıkış Yap
          <div class="card">
          </div>
        </button></a>
      </div>
    ...
    <div class="row mt-4">
      ...
      <div class="card-body">
        ...
        <tr id="{{$value->ogr_no}}">
        <td> {{$value->eposta}}</td>
        <td>{{ base64_decode($sifre) }}</td>
        <td> <img src="\assets\ogrenci\{{ $value->fotograf }}" style="width: 25px; heigth:25px;"></td>
        <td>{{ $value->telefon }}</td>
        <td><a class="btn btn-dark"  href="/ogrenci-profil/{{$id}}">güncelle</a></td>
        </tr>
        </tbody>
          </table>
          </div>
            ...
            <div class="row">
              <div class="col-lg">
                <div class="d-flex flex-column h-50">
                    @php
                        $proje = \App\Models\Project::where('ogr_no',$value->ogr_no)->orderBy('id','desc')->first();
                        $rapor = \App\Models\Report::where('ogr_no',$value->ogr_no)->orderBy('id','desc')->first();
                    @endphp
                  <a href="/projeonerisi/{{$id}}" class="btn btn-primary">Proje Önerisi</a>
                  <a href="/projeraporu/{{$id}}" class="btn btn-primary"@if($proje == null or $proje->durum_id !='3' ) hidden @endif>Proje Raporu</a>
                  <a href="/tezraporu/{{$id}}" class="btn btn-primary"@if($rapor == null or $rapor->durum_id !='3') hidden @endif>Tez Raporu</a>
                </div>
              </div>
            </div>
          </div>
