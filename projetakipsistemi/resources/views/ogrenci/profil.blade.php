...
<aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-left ms-3"
    ...
    <div class="sidenav-header">
        ...
        <a class="navbar-brand m-0" href="/ogrenciekran/{{$ogr_no}}">
            <img src="../assets/img/logo-ct.png" class="navbar-brand-img h-100" alt="...">
            <span class="ms-1 font-weight-bold">{{$ogr_no}}</span>
        </a>
    </div>
    ...
    <div class="collapse navbar-collapse w-auto" id="sidenav-collapse-main">
        <ul class="navbar-nav">
            ...
            <li class="nav-item pb-2">
                <a  class="nav-link {{ Route::currentRouteName() == 'dashboard' ? 'active' : '' }} " href="/danismanbilgisi/{{$ogr_no}}">
                    <div
                        class="icon icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
                ...
                <div class="row gx-4">
                    <div class="col-auto ">
                        <div class="avatar avatar-xl position-relative">
                            @if(!empty($guncelle->fotograf))
                            <img src="/assets/ogrenci/{{ $guncelle->fotograf}}" alt="..." class="w-100 border-radius-lg shadow-sm">
                        @endif
                        </div>
                    </div>
        ...
        <div class="container-fluid py-4">
            <div class="card">
                ...
                <div class="card-body pt-4 p-3">
                    @php
                      $sifre = \App\Models\System::where('kullanici_no',$ogr_no)->value('sifre');
                      $sifre = base64_decode($sifre);
                    @endphp
                    <!-- gerçek kodda 200. satır kaydettikten sonra "kaydedildi" mesajının çıkmasını sağlayan kod !-->

                    <form action="{{ route('post.ogrenci.profil') }}" method="post" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
                             <input type="hidden" id="ogr_no" name="ogr_no" value="{{$ogr_no}}">
                            <div class="col-md-6 mb-3">
                                <label for="">Telefon Numarası</label>
                                <input type="text" name="telefon" class="form-control" id="telefon" maxlength="11" value="{{$guncelle->telefon}}">
                            </div>
                            <div class="col-md-6 mb-3">
                                <label for=""> e posta</label>
                                <input type="text" name="eposta" class="form-control" id="eposta"   value="{{$guncelle->eposta}}">
                            </div>
                            <div class="col-md-6 mb-3">
                                <label for="">Öğrenci Fotoğrafı</label>
                            <input type="file" name="fotograf" class="form-control"  id="fotograf" >
                            </div>
                            <div class="col-md-6 mb-3">
                                <label for="">şifre</label>
                                <input type="text" name="sifre" id="sifre" class="form-control" value="{{$sifre}}">
                            </div>
